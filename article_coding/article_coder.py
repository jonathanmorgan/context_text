...
class ArticleCoder( BasicRateLimited ):
    ...
    def load_config_properties( self, *args, **kwargs ):
        ...
        for property_name in config_prop_list:
            ...
            # retrieve property
            property_value = Config_Property.get_property_value( config_application, property_name, default_IN = None )
    ...
    def lookup_person( self,
                       ...
                       update_article_person_name_fields_IN = False,
                       *args,
                       **kwargs ):
        ...
        #------------------------------------------------------------------------
            ...
            if ( person_instance is not None ):
                ...
                # make sure there is always a verbatim name and lookup name

                # update verbatim_name (either because we've been asked to, or
                #     because it is empty?
                if (
                    ( update_article_person_name_fields_IN == True )
                    or (
                        ( instance_OUT.verbatim_name is None )
                        or ( instance_OUT.verbatim_name == "" )
                    )
                ):

                    # no - use full name.
                    instance_OUT.verbatim_name = full_name_IN

                #-- END check if need to set verbatim_name --#

                # update lookup_name (either because we've been asked to, or
                #     because it is empty?
                if (
                    ( update_article_person_name_fields_IN == True )
                    or (
                        ( instance_OUT.lookup_name is None )
                        or ( instance_OUT.lookup_name == "" )
                    )
                ):

                    # no - use full name.
                    instance_OUT.lookup_name = full_name_IN

                #-- END check if need to set lookup_name --#
